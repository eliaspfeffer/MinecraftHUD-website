...
const heroRevealTimer=setTimeout(()=>finishIntro(),2000);
let introFrame=0;
...
const cursorXpDownload=document.querySelector('.cursor-xp-download'),cursorXpSwarm=document.querySelector('.cursor-xp-swarm');if(cursorXpDownload&&cursorXpSwarm){let active=false,frame=0,pointer={x:0,y:0};const particles=[];for(let i=0;i<5;i++){const orb=document.createElement('img');orb.src='assets/app/xp-orb.png';orb.alt='';cursorXpSwarm.appendChild(orb);particles.push({orb,x:0,y:0,vx:0,vy:0,phase:Math.random()*Math.PI*2,speed:.45+Math.random()*.55,range:25+Math.random()*28})}function center(){return{x:cursorXpDownload.offsetWidth/2,y:cursorXpDownload.offsetHeight/2}}function draw(p,scale=1){p.orb.style.transform=`translate3d(${p.x-11}px,${p.y-11}px,0) scale(${scale})`}function tick(time){if(!active)return;particles.forEach((p,i)=>{const t=time*.001*p.speed+p.phase,targetX=pointer.x+Math.sin(t*1.7+i)*p.range+Math.sin(t*3.1)*9,targetY=pointer.y+Math.cos(t*1.19+i*1.4)*p.range*.62+Math.sin(t*2.3+i)*12;p.vx=(p.vx+(targetX-p.x)*.018)*.91;p.vy=(p.vy+(targetY-p.y)*.018)*.91;p.x+=p.vx;p.y+=p.vy;const front=Math.sin(t+i*1.8)>-.12;p.orb.style.zIndex=front?'4':'2';p.orb.style.opacity=front?'1':'.52';draw(p,front?1:.68)});frame=requestAnimationFrame(tick)}cursorXpDownload.addEventListener('pointerenter',event=>{active=true;const r=cursorXpDownload.getBoundingClientRect(),c=center();pointer={x:event.clientX-r.left,y:event.clientY-r.top};particles.forEach((p,i)=>{p.orb.getAnimations().forEach(a=>a.cancel());p.x=c.x;p.y=c.y;p.vx=(Math.random()-.5)*3;p.vy=(Math.random()-.5)*3;p.orb.style.zIndex='2';draw(p,.12);p.orb.animate([{opacity:0,transform:`translate3d(${c.x-11}px,${c.y-11}px,0) scale(.12)`},{opacity:.85,offset:.55},{opacity:1}],{duration:420+i*55,easing:'cubic-bezier(.2,.8,.25,1)',fill:'none'})});cancelAnimationFrame(frame);frame=requestAnimationFrame(tick)});cursorXpDownload.addEventListener('pointermove',event=>{const r=cursorXpDownload.getBoundingClientRect();pointer={x:event.clientX-r.left,y:event.clientY-r.top}});cursorXpDownload.addEventListener('pointerleave',()=>{active=false;cancelAnimationFrame(frame);const c=center();particles.forEach((p,i)=>{const current=getComputedStyle(p.orb).transform;p.orb.style.zIndex='2';p.orb.animate([{transform:current,opacity:getComputedStyle(p.orb).opacity},{transform:`translate3d(${c.x-11}px,${c.y-11}px,0) scale(.08)`,opacity:0}],{duration:420+i*45,easing:'cubic-bezier(.55,0,.8,.2)',fill:'forwards'})})})}
