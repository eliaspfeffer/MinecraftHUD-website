...
(function buildCornerSteve(){
 ...
 (function animate(){const nervous=document.getElementById('corner-steve').classList.contains('nervous'),shake=nervous?Math.sin(Date.now()/35)*18:0;yaw+=(ty-yaw)*.1;pitch+=(tp-pitch)*.1;container.style.transform=`translateZ(18px) rotateY(${(-18+yaw*.45).toFixed(2)}deg) rotateX(${(8+pitch*.15).toFixed(2)}deg)`;headPivot.style.transform=`translate3d(48px,48px,0) rotateY(${(yaw*.65+shake).toFixed(2)}deg) rotateX(${pitch.toFixed(2)}deg)`;requestAnimationFrame(animate)})();
})();

document.addEventListener('pointermove',event=>{document.querySelectorAll('.steve-stage .steve-model').forEach(steve=>{const r=steve.getBoundingClientRect(),x=(event.clientX-(r.left+r.width/2))/Math.max(innerWidth,.1),y=(event.clientY-(r.top+r.height/2))/Math.max(innerHeight,.1);steve.style.transform=`translateZ(24px) rotateY(${(-x*85).toFixed(1)}deg) rotateX(${(y*45).toFixed(1)}deg)`})});
