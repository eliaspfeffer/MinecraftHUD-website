...
function unlockPage(){body.classList.remove('intro-active');body.classList.add('page-ready');intro.classList.add('scroll-unlocked')}
const heroRevealTimer=setTimeout(unlockPage,2000);
let introFrame=0;
...
document.querySelectorAll('.cursor-xp-download').forEach(download=>{const swarm=download.querySelector('.cursor-xp-swarm');if(!swarm)return;let active=false,frame=0,pointer={x:0,y:0};const particles=[];for(let i=0;i<5;i++){const orb=document.createElement('img');orb.src='assets/app/xp-orb.png';orb.alt='';swarm.appendChild(orb);particles.push({orb,x:0,y:0,vx:0,vy:0,front:false,phase:Math.random()*Math.PI*2,speed:.7+Math.random()*.45,range:18+Math.random()*14})}const center=()=>({x:download.offsetWidth/2,y:download.offsetHeight/2}),draw=(p,scale=1)=>{p.orb.style.transform=`translate3d(${p.x-11}px,${p.y-11}px,0) scale(${scale})`};function tick(time){if(!active)return;particles.forEach((p,i)=>{const t=time*.001*p.speed+p.phase,targetX=pointer.x+Math.sin(t*2.1+i*1.31)*p.range+Math.sin(t*4.3+i)*4,targetY=pointer.y+Math.cos(t*1.73+i*1.67)*p.range*.72+Math.sin(t*3.2+i)*5;p.vx=(p.vx+(targetX-p.x)*.11)*.72;p.vy=(p.vy+(targetY-p.y)*.11)*.72;p.x+=p.vx;p.y+=p.vy;const outside=p.x<0||p.x>download.offsetWidth||p.y<0||p.y>download.offsetHeight;if(outside&&!p.front&&Math.sin(t*1.4+i*1.8)>-.12)p.front=true;p.orb.style.zIndex=p.front?'4':'2';p.orb.style.opacity=p.front?'1':'.7';draw(p,p.front?1:.72)});frame=requestAnimationFrame(tick)}download.addEventListener('pointerenter',event=>{active=true;const r=download.getBoundingClientRect(),c=center();pointer={x:event.clientX-r.left,y:event.clientY-r.top};particles.forEach((p,i)=>{p.orb.getAnimations().forEach(a=>a.cancel());p.x=c.x;p.y=c.y;p.vx=(Math.random()-.5)*2;p.vy=(Math.random()-.5)*2;p.front=false;p.orb.style.zIndex='2';draw(p,.12);p.orb.animate([{opacity:0},{opacity:1}],{duration:320+i*45,easing:'ease-out'})});cancelAnimationFrame(frame);frame=requestAnimationFrame(tick)});download.addEventListener('pointermove',event=>{const r=download.getBoundingClientRect();pointer={x:event.clientX-r.left,y:event.clientY-r.top}});download.addEventListener('pointerleave',()=>{active=false;cancelAnimationFrame(frame);const c=center();particles.forEach((p,i)=>{const current=getComputedStyle(p.orb).transform;p.front=false;p.orb.style.zIndex='2';p.orb.animate([{transform:current,opacity:getComputedStyle(p.orb).opacity},{transform:`translate3d(${c.x-11}px,${c.y-11}px,0) scale(.08)`,opacity:0}],{duration:360+i*40,easing:'cubic-bezier(.55,0,.8,.2)',fill:'forwards'})})})});
